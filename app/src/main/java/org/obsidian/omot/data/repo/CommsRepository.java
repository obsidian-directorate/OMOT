...
public class CommsRepository {
    ...
    public Result<Boolean> sendMessage(String messageId, String senderId, String recipientId, String messagePlain, long sentAt) {
        try {
            ...
            if (!ok) {
                Logs.write(senderId, "SEND_MESSAGE_FAIL", "MsgID: " + messageId + " -> " + recipientId);
                return Result.failure(new Exception("Failed to insert message."));
            }
            Logs.write(senderId, "SEND_MESSAGE_SUCCESS", "MsgID: " + messageId + " -> " + recipientId);
